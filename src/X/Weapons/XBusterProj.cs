...
public class XUnpoProjBase : Projectile {
	public XUnpoProjBase(Actor owner, Point pos, int xDir, ushort netId, DireccionDisparo direccion, int damage, string sprite, bool sendRpc = false)
		: base(pos, xDir, owner, sprite, netId) {
		...
		vel.x = GetVelX(direccion, xDir, charState);
		vel.y = GetVelY(direccion);
		angle = GetAngulo(direccion, xDir, charState);
	...
	public class XUnpoProjMaximo : XUnpoProjBase {
		public XUnpoProjMaximo(Actor owner, Point pos, int xDir, ushort netId, DireccionDisparo direccion, bool sendRpc = true)
			: base(owner, pos, xDir, netId, direccion, 3, "buster_unpo", sendRpc) { }
	}



	// **Métodos para calcular velocidad y ángulo**
	public static float GetVelX(DireccionDisparo direccion, int xDir, CharState charState) {
	...
	public static float GetAngulo(DireccionDisparo direccion, int xDir, CharState charState) {
		// Si el personaje está en WallSlide, invertimos el ángulo
		if (charState is WallSlide) {
			return direccion switch {
				DireccionDisparo.Arriba => 90f,  // Invertido
				DireccionDisparo.Abajo => -90f, // Invertido
				DireccionDisparo.Frente => 180f, // Disparo hacia atrás
				_ => 0f
			};
		}

		// Comportamiento normal cuando no está en WallSlide
		return direccion switch {
			DireccionDisparo.Arriba => -90f,
			DireccionDisparo.Abajo => 90f,
